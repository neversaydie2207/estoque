...
public class EntradasProdutos2 implements Serializable {
	...
	public List<EntradaProduto> todos() {
		try {
			return manager.createQuery("from EntradaProduto ep order by ep.id ASC", EntradaProduto.class).getResultList();
	...
	public EntradaProduto pesquisarPorId(Double Id) {
		if (Id != null) {
			EntradaProduto entradaProdutoRegistrado = porId(Id);
			...
			throw new NegocioException("Por favor, informe o ID VÁLIDO para esta pesquisa!");
		}
	}
	...
	public EntradaProduto porId(Double id) {
		try {
			return manager.createQuery("from EntradaProduto ep where ep.id = :id", EntradaProduto.class)
