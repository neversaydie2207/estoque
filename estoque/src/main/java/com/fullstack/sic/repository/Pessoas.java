...
public class Pessoas implements Serializable {
	...
	public List<Pessoa> todos() {
		try {
			return manager.createQuery("from Pessoa p order by p.id ASC", Pessoa.class).getResultList();
	...
	public Pessoa pesquisarPorCodigo(String codigo) {
		if (codigo != null) {
			Pessoa pessoaRegistrado = porCodigo(codigo);

			if (pessoaRegistrado != null) {
				return pessoaRegistrado;
			} else {
				return null;
			}
	...
	public Pessoa porCodigo(String codigo) {
		try {
			return manager.createQuery("from Pessoa p where p.codigo = :codigo", Pessoa.class)
